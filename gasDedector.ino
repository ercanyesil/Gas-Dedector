...
const char* password = "4d52321876";
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      // ... and resubscribe
      client.subscribe("ortamLed");
    } 
    else if (client.connect(clientId.c_str())) {
      Serial.println("connected");
      // Once connected, publish an announcement...
...
void setup() {
  ...
  pinMode(yesil_led,OUTPUT);     // Initialize the BUILTIN_LED pin as an output
  Serial.begin(115200);
  setup_wifi();
...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    Serial.println(msg);
    client.publish("gasDedector", msg);
  }
}
